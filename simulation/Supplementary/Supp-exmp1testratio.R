...
Result <- foreach(iter = 1:nr, .combine = "rbind", .packages = c("MASS", "isotree", "randomForest"), .errorhandling = "remove")%dopar% {
  ...
  for (pi0 in pi0array) {
    ...
    data <- rbind(data, data.frame(FDP = FDP_new, POWER = power_new, method = 'ECOT-bi', n = nfull, sig = sig, pi0 = pi0))
    ...
    data <- rbind(data, data.frame(FDP = FDP_int, POWER = power_int, method = 'Integ', n = nfull, sig = sig, pi0 = pi0))
   ...
   data <- rbind(data, data.frame(FDP = FDP_int_re, POWER = power_int_re, method = 'ECOT-oc', n = nfull, sig = sig, pi0 = pi0))
  ...
  ###---method selection---###
    ...
    data <- rbind(data, data.frame(FDP = FDP_sel, POWER = power_sel, method = 'ECOT-as', n = nfull, sig = sig, pi0 = pi0))  
  }
