...
n0 <- 400
n1 <- 100
marray <- c(200, 500, 1000, 2000, 5000)
...
Result <- foreach(iter = 1:nr, .combine = "rbind", .packages = c("MASS", "isotree", "randomForest"), .errorhandling = "remove")%dopar% {
  ...
  for (m in marray) {
    ...
    data <- rbind(data, data.frame(FDP = FDP_cp, POWER = power_cp, method = 'CP-oc', n = n0, sig = sig, m = m))
    ...
    data <- rbind(data, data.frame(FDP = FDP_oc, POWER = power_oc, method = 'FullND', n = n0, sig = sig, m = m))
    ...
    data <- rbind(data, data.frame(FDP = FDP_ada, POWER = power_ada, method = 'AdaDet', n = n0, sig = sig, m = m))
    ...
    data <- rbind(data, data.frame(FDP = FDP_BIN, POWER = power_BIN, method = 'CP-bi', n = n0, sig = sig, m = m))
    ...
    data <- rbind(data, data.frame(FDP = FDP_new, POWER = power_new, method = 'ECOT-bi', n = n0, sig = sig, m = m))
    ...
    data <- rbind(data, data.frame(FDP = FDP_int, POWER = power_int, method = 'Integ', n = n0, sig = sig, m = m))
   ...
   data <- rbind(data, data.frame(FDP = FDP_int_re, POWER = power_int_re, method = 'ECOT-oc', n = n0, sig = sig, m = m))
  ...
  for (i in 1:m) {
    ...
    data <- rbind(data, data.frame(FDP = FDP_sel, POWER = power_sel, method = 'ECOT-as', n = n0, sig = sig, m = m))
  }
...
save(Result, file = 'Result_1_m.RData')
