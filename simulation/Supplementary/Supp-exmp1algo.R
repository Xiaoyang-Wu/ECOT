...
sig <- 1
narray <- seq(150, 750, 100)


nr <- 500
cl <- makeCluster(100)
registerDoParallel(cl)
Result <- foreach(iter = 1:nr, .combine = "rbind", .packages = c("MASS", "kernlab"), .errorhandling = "remove")%dopar% {
  ...
  for (nfull in narray) {
    ...
    model0_IOF <- ksvm(X0_train, type = "one-svc", kernel = "rbfdot", nu = 0.5)
    model1_IOF <- ksvm(X1_train, type = "one-svc", kernel = "rbfdot", nu = 0.5)
    model0_new <- ksvm(rbind(X0, X_test), type = "one-svc", kernel = "rbfdot", nu = 0.5)
    
    model_RF <- ksvm(y~., data = data.frame(x = rbind(X0_train, X0_cal, X_test), y = factor(c(rep(0, n/2), rep(1, n/2+m)))), type = 'C-svc', prob.model = T)
    model_RF_B <- ksvm(y~., data = data.frame(x = rbind(X0_train, X1), y = factor(c(rep(0, n/2), rep(1, n1)))), type = 'C-svc', prob.model = T)
    model_RF_new <- ksvm(y~., data = data.frame(x = rbind(X0, X_test, X1), y = factor(c(rep(0, n+m), rep(1, n1)))), type = 'C-svc', prob.model = T)
    
    
    s0_cal <- predict(model0_IOF, X0_cal, type = 'decision')
    s0_cal1 <- predict(model1_IOF, X0_cal, type = 'decision')
    s1_cal <- predict(model1_IOF, X1_cal, type = 'decision')
    s0_new_cal1 <- predict(model1_IOF, X0, type = 'decision')
    s0_test <- predict(model0_IOF, X_test, type = 'decision')
    s1_test <- predict(model1_IOF, X_test, type = 'decision')
    s0_new <- predict(model0_new, X_test, type = 'decision')
    s0_new_cal <- predict(model0_new, X0, type = 'decision')
    ...
    data <- rbind(data, data.frame(FDP = FDP_BIN, POWER = power_BIN, method = 'CP-bi', n = nfull, sig = sig))
    ...
    data <- rbind(data, data.frame(FDP = FDP_new, POWER = power_new, method = 'ECOT-bi', n = nfull, sig = sig))
    ...
    data <- rbind(data, data.frame(FDP = FDP_int, POWER = power_int, method = 'Integ', n = nfull, sig = sig))
   ...
   data <- rbind(data, data.frame(FDP = FDP_int_re, POWER = power_int_re, method = 'ECOT-oc', n = nfull, sig = sig))
  ...
  data <- rbind(data, data.frame(FDP = FDP_sel, POWER = power_sel, method = 'ECOT-as', n = nfull, sig = sig))
  }
...
save(Result, file = 'Result_1_algo.RData')
