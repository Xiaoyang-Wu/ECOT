...
narray <- c(100, 250, 400, 550, 700, 850, 1000)
...
Result <- foreach(iter = 1:nr, .combine = "rbind", .packages = c("MASS", "isotree", "randomForest"), .errorhandling = "remove")%dopar% {
  ...
  for (i in 1:m) {
    ...
    for (j in 1:m){
      ...
      if(j==1){
        pval_oc_sel <- c(0, pval_oc_sel)
      }else if(j==m){
        pval_oc_sel <- c(pval_oc_sel, 0)
      }else {
        pval_oc_sel <- c(pval_oc_sel[1:(j-1)], 0, pval_oc_sel[j:(m-1)])
      }
...
save(Result, file = 'Result_2_n.RData')
