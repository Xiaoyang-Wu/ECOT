...
dataori <- rbind(read.csv('train_shuttle.csv', header = F), read.csv('test_shuttle.csv', header = F))
data_clean <- dataori[dataori$V10==1, ]
data_out <- dataori[dataori$V10!=1, ]
...
Result <- foreach(iter = 1:nr, .combine = "rbind", .packages = c("MASS", "isotree", "e1071", "randomForest"), .errorhandling = "remove")%dopar% {
  ...
  data <- rbind(data, data.frame(t = t_ocs_full, FDP = FDP_ocsfull, POWER = power_ocsfull, method = 'OCS-full', n = nfull, alpha = alpha))
...
save(Result, file = 'Shuttle-re.RData')
