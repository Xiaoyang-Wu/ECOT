...
dataori <- read.csv('covtype.csv', header = F)[,c(1:10, 55)]
data_clean <- dataori[dataori$V55==2, ]
data_out <- dataori[dataori$V55==4, ]
...
Result <- foreach(iter = 1:nr, .combine = "rbind", .packages = c("MASS", "isotree", "randomForest"), .errorhandling = "remove")%dopar% {
  ...
  data <- rbind(data, data.frame(FDP = FDP_new, POWER = power_new, method = 'ECOS-bi'))
  ...
  data <- rbind(data, data.frame(FDP = FDP_int_re, POWER = power_int_re, method = 'ECOS-oc'))
  ...
  pval_sel <- rep(NA, m)
  for (j in 1:m){
    rem <- which.max(c(Vn_cal, Vn_test[j]))
    rejnum_each <- rep(NA, 6)
    Vn_cal_sel <- c(Vn_cal, Vn_test[j])
    Vn_cal_sel <- Vn_cal_sel[-rem]
  ...
  data <- rbind(data, data.frame(FDP = FDP_sel, POWER = power_sel, method = 'ECOS-as'))
  
  return(data)
}
stopCluster(cl)

save(Result, file = 'covtype.RData')
